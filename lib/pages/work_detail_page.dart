...
import '../common/tools.dart' show openFileExplorerAndSelectFile;
...
class _WorkDetailPageState extends State<WorkDetailPage> {
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      rightWidget: ContextMenuArea(
          builder: (context) => [
                ListTile(
                  title: Text('Open in file explorer'),
                  onTap: () {
                    openFileExplorerAndSelectFile(widget.workInfo.type ==
                            'novel'
                        ? '${widget.controller.hostPath}${widget.workInfo.coverImagePath!}'
                        : '${widget.controller.hostPath}${widget.workInfo.imagePath![0]}');
                  },
                ),
                ListTile(
                  ...
                  onTap: () {
                    ...
                    if (widget.workInfo.type == 'novel') {
                      url += "novel/show.php?id=";
                    } else {
                      url += "artworks/";
                    }
                    launchUrl(Uri.parse("$url${widget.workInfo.id}")).then(
                        (success) => success
                            ? {}
                            : {
                                resultDialog(
                                    widget.workInfo.title.toLowerCase(),
                                    success)
                              });
                  },
                )
              ...
              children: [
                ...
                Positioned(
                    ...
                    child: IconButton(
                      onPressed: () {
                        if (index.value <
                            widget.workInfo.imagePath!.length - 1) {
                          index.value += 1;
                        }
                      },
